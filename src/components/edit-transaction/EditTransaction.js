...
const EditTransaction = ({transaction, onChangeTransaction, onDeleteTransaction, onClose}) => {
    return (
        <Modal onClose={onClose} classNames='edit-transaction'>
            <Filters
                transaction={transaction}>
                {(transaction) => <div>
                    <Button onClick={() => onChangeTransaction(transaction)}>Изменить Транзакцию</Button>
                    <Button onClick={() => onDeleteTransaction(transaction)}>Удалить Транзакцию</Button>
                </div>}
            </Filters>
        </Modal>
    );
